...
export default function AddExpensePage() {
    ...
    const addExpenseMutation = useMutation({
        ...
        onMutate: async (newExpenseData) => {
            ...
            // ── الخروج المبكر **بلا إنترنت فقط** ───────────────────────────────
            // وعد Firestore بالكتابة **لا يُحسم إلا بتأكيد الخادم**. فبلا إنترنت كان
            // المصروف يُحفظ محلياً فعلاً بينما تبقى الشاشة معلّقة وزرّها يدور بلا
            // نهاية، فيظنّ المستخدم أن الحفظ فشل ويعيده — فيصير مصروفان.
            //
            // ⚠️ ولماذا **بلا إنترنت فقط** لا دائماً: المغادرة تُفكِّك هذا المكوّن،
            // وبتفكيكه تتوقف `onError`/`onSuccess`/`onSettled` عن العمل (هكذا يعمل
            // React Query: توابع `useMutation` مرتبطة بالمكوّن الحيّ). فلو غادرنا
            // دائماً لضاع **التراجع عن التحديث المتفائل** عند فشل حقيقي أثناء
            // الاتصال — أي يبقى مصروف وهمي معروضاً لم يُحفظ. ومع الاتصال لا مشكلة
            // أصلاً: الوعد يُحسم خلال لحظة. فالسلوك المتصل **لم يتغيّر بحرف**.
            //
            // وبلا إنترنت لا يوجد فشل يُتراجَع عنه: Firestore يصفّ الكتابة محلياً
            // ويرفعها عند عودة الشبكة.
            if (typeof navigator !== 'undefined' && !navigator.onLine) {
                form.reset();
                // إلى الرئيسية لا إلى شاشة المصاريف: الرئيسية وحدها مضمونة الحفظ
                // دائماً (start-url)، وغيرها قد يكون غير محفوظ فيصطدم المستخدم
                // بصفحة «هذه الشاشة تحتاج إنترنت» بعد حفظ ناجح.
                router.push('/');
            }
        ...
        onSuccess: (_, variables) => {
            ...
            // المسار المتصل كما كان تماماً: تنظيف النموذج ثم الانتقال بعد تأكيد
            // الخادم. (حالة عدم الاتصال غادرت مبكراً في `onMutate` — الشرح هناك،
            // وهذا السطر لا يُنفَّذ لها لأن المكوّن يكون قد فُكِّك.)
            form.reset();
