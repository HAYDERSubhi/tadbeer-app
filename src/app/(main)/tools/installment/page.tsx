...
function payDayFromDate(dateStr: string): number {
...
function nextPaymentDate(plan: InstallmentPlan): string {
  ...
  const rawMonth = start.getUTCMonth() + plan.paidCount;
  const year     = start.getUTCFullYear() + Math.floor(rawMonth / 12);
  const month    = rawMonth % 12;
  const lastDay  = new Date(Date.UTC(year, month + 1, 0)).getUTCDate();
  const day      = Math.min(plan.paymentDay, lastDay);
...
function dueDateLabel(days: number): string {
  ...
  return new Date(Date.now() + days * 86400000)
    .toLocaleDateString('ar-IQ', { day: 'numeric', month: 'short' });
}
...
function PlanCard({ plan, onPay, onDelete }: {
  ...
  // FIX #6: تأكيد قبل تسجيل الدفع
  const [confirmPay, setConfirmPay] = useState(false);

  const progress  = Math.round((plan.paidCount / plan.months) * 100);
  // FIX #3: "مدفوع" = الأقساط فقط. الدفعة الأولى تُعرض منفصلة إذا وُجدت
  const paidInstallments = plan.monthlyPayment * plan.paidCount;
    ...
    <div className={`bg-card border rounded-2xl px-4 py-3 ${borderCls}`}>
      ...
      <div className="flex items-start justify-between mb-2">
        ...
        <div className="flex items-center gap-1.5">
          {plan.isCompleted ? (
            <span className="text-[10px] bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 px-2 py-0.5 rounded-lg">
              مكتملة
            </span>
          ) : days !== null ? (
            // FIX #7: "اليوم" بلون مميز (primary) بدل الأصفر
      ...
      {/* أرقام */}
      <div className={`grid gap-2 mb-2.5 text-center ${plan.downPayment > 0 ? 'grid-cols-4' : 'grid-cols-3'}`}>
        ...
        {plan.downPayment > 0 && (
          <div className="bg-muted/40 rounded-xl py-2">
            <p className="text-xs font-bold">{fmt(plan.downPayment)}</p>
            <p className="text-[10px] text-muted-foreground">دفعة أولى</p>
          </div>
        )}
        <div className="bg-emerald-50 dark:bg-emerald-900/20 rounded-xl py-2">
          <p className="text-xs font-bold text-emerald-700 dark:text-emerald-400">{fmt(paidInstallments)}</p>
      ...
      {/* FIX #6: زر الدفع مع تأكيد */}
      {!plan.isCompleted && (
        confirmPay ? (
          <div className="flex gap-2">
            <button onClick={() => setConfirmPay(false)}
              className="flex-1 py-2.5 rounded-xl border border-border text-xs text-muted-foreground">
              إلغاء
            </button>
            <button onClick={() => { setConfirmPay(false); onPay(); }}
              className="flex-1 py-2.5 rounded-xl bg-primary text-primary-foreground text-xs font-semibold">
              تأكيد دفع القسط {plan.paidCount + 1}
            </button>
          </div>
        ) : (
...
export default function InstallmentPage() {
  ...
  const addMutation = useMutation({
    mutationFn: (data: Omit<InstallmentPlan,'id'|'uid'|'createdAt'>) =>
